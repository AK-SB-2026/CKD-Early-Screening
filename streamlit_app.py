...
st.caption(
    "Two-Stage CKD Screening System — Early Risk Screening → Clinical Severity Screening"
)

st.markdown(
    """
    **Workflow:** 🩺 Early Screening → 📋 Interpretation → 📊 Analytics →
    📈 Plots → 🏥 Insurance Report → 📚 About → 🏥 Clinical Screening
    """
)
...
#
# Access to "Clinical Screening" is gated behind completion of the
# "Early Screening" prediction: the tabs for it are only populated once
# st.session_state["prediction"] exists (i.e. after a screening has been run
# in the CKD Prediction tab further down this script).
#
# =============================================================================

# TWO-STAGE APPLICATION NAVIGATION
# Clinical Screening is deliberately unavailable until Early Screening
# produces a prediction. Streamlit tabs cannot be disabled, so a gated
# section selector is used to enforce the workflow.
# ---------------------------------------------------------------------------
early_completed = "prediction" in st.session_state

section_options = (
    ["1️⃣ Early Screening", "2️⃣ Clinical Screening"]
    if early_completed
    else ["1️⃣ Early Screening"]
)
...
selected_section = st.radio(
    ...
    section_options,
    horizontal=True,
    key="application_section"
)

if early_completed:
    st.success(
        "✅ Early Screening completed. Clinical Screening is now unlocked."
    )
else:
    st.info(
        "🔒 Complete **Early Screening → CKD Prediction** first. "
        "Clinical Screening will unlock automatically after a prediction."
    )
...
with tab_home:
    ...
    with struct_col2:
        ...
        st.markdown(
            ...
            CKD-**severity** classification, developed separately. This
            section unlocks only after an Early Screening prediction has
            been generated.
            """
        )
    ...
    st.info(
        """
        🔒 **Note:** The **Clinical Screening** section is locked until a
        patient has been screened in the **Early Screening → CKD
        Prediction** tab. This ensures every patient has an early-screening
        risk result before moving on to clinical/severity assessment.
        """
    )
...
with tab_prediction:
    ...
    if run_screening:
        ...
        if not MODEL_LOADED:
            ...
            try:
                ...
                # The top-level section selector is created before this
                # prediction button. Request one controlled rerun so the
                # newly unlocked Clinical Screening section is selected
                # automatically. This removes the old two-click behaviour.
                st.session_state["_clinical_unlocked"] = True
                ...
                st.success(
                    "CKD screening completed successfully."
                )
                ...
                if int(prediction) == 1:
                    ...
                    st.error(
                        "⚠️ HIGHER CKD RISK DETECTED"
                    )

                    st.write(
                        """
                        The model classified this screening profile
                        as **CKD Risk**.
                        """
                    )
                ...
                st.info(
                    """
                    The SVM decision score indicates the position of the
                    patient profile relative to the model's classification
                    boundary. It is **not a clinical probability**.
                    """
                )
                st.rerun()
    ...
    # =====================================================================
    # PERSISTENT IMMEDIATE MEDICAL ANALYSIS
    # =====================================================================
    # The first successful Early Screening prediction unlocks Clinical
    # Screening but does NOT navigate there. This medical profile remains
    # visible on the Early Screening prediction page after the controlled
    # Streamlit rerun.
    if "prediction" in st.session_state and "input_data" in st.session_state:
        ...
        st.caption("Patient-level medical profile from the Early Screening information.")

        c1, c2 = st.columns(2)
        with c1:
            st.metric("Early Screening Result", "CKD Risk" if prediction_persistent == 1 else "No CKD Risk")
        with c2:
            st.metric("SVM Decision Score", f"{decision_score_persistent:.4f}")
        ...
        st.info(
            "This is the detailed medical profile from Early Screening. "
            "The partner's CKD severity model remains a separate Clinical Screening stage."
        )
...
with tab_interpretation:
    ...
    if "prediction" not in st.session_state:
        ...
        if int(prediction) == 1:
            ...
            risk_description = (
                "The final Linear SVM classified the "
                "submitted screening profile as CKD risk."
            )
            ...
            risk_description = (
                "The final Linear SVM classified the "
                "submitted screening profile as no CKD risk."
            )
...
with tab_ckd_visuals:
    ...
    if visualisation_section == "Model Comparison":
        ...
        fig_metrics, ax_metrics = plt.subplots(figsize=(11, 6))
        ...
        for i, model in enumerate(NOTEBOOK_MODEL_RESULTS["Model"]):
            ...
            ax_metrics.bar(x + (i-1)*width, vals, width, label=model)
        ax_metrics.set_xticks(x)
        ax_metrics.set_xticklabels(metric_cols)
        ...
        for ax, (model, cmv) in zip(cm_axes, NOTEBOOK_CONFUSION_MATRICES.items()):
            ax.imshow(cmv)
            ax.set_title(model)
            ax.set_xlabel("Predicted")
            ax.set_ylabel("Actual")
            ax.set_xticks([0, 1]); ax.set_yticks([0, 1])
            ax.set_xticklabels(["No CKD", "CKD"]); ax.set_yticklabels(["No CKD", "CKD"])
            for i in range(2):
                for j in range(2):
                    ax.text(j, i, cmv[i, j], ha="center", va="center")
        plt.tight_layout()
        st.pyplot(cm_fig)
        plt.close(cm_fig)
...
with tab_insurance:
    ...
    if (
        ...
        if "Health_Insurance" in insurance_analysis.columns:
            ...
            fig_insurance_dist, ax_insurance_dist = plt.subplots(
                figsize=(9, 6)
            )


            ax_insurance_dist.bar(
                insurance_counts.index,
            ...
            ax_insurance_dist.set_xlabel(
                "Health Insurance Status",
                fontsize=12
            )


            ax_insurance_dist.set_ylabel(
                "Number of Patients",
                fontsize=12
            )


            ax_insurance_dist.set_title(
...
def load_severity_model():
    base = Path(__file__).resolve().parent
    model_p = base / "CKD_Severity_XGBoost.pkl"
    scaler_p = base / "scaler.pkl"
    imputer_p = base / "num_imputer.pkl"
    
    if not (model_p.exists() and scaler_p.exists() and imputer_p.exists()):
        # Try current working directory as fallback
        cwd = Path.cwd()
        model_p = cwd / "CKD_Severity_XGBoost.pkl"
        scaler_p = cwd / "scaler.pkl"
        imputer_p = cwd / "num_imputer.pkl"
        if not (model_p.exists() and scaler_p.exists() and imputer_p.exists()):
            raise FileNotFoundError("Clinical severity model files are missing.")
    
    model = joblib.load(model_p)
    scaler_obj = joblib.load(scaler_p)
    imputer_obj = joblib.load(imputer_p)
...
if main_clinical_screening:
    ...
    if "prediction" not in st.session_state:
        ...
        with tab_clin_predict:
            ...
            st.header("🩺 CKD Clinical Severity Screening")
        ...
        with tab_clin_interpret:
            ...
            st.header("📋 Interpretation & Outcomes")

            if "severity_label" not in st.session_state:

                st.warning(
                    "No CKD severity result is available yet."
                )
...
# =============================================================================
# END OF CLINICAL SCREENING SECTION
# =============================================================================
