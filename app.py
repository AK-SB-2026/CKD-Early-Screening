...
# =============================================================================
#
# Insurance Intelligence is integrated directly from the Early Screening
# validation outputs contained in early_screening(2).ipynb.
#
# IMPORTANT:
#   - No second dataset is searched for.
#   - No second train/test split is performed.
#   - No second SVM is trained.
#   - This section presents the already-computed Early Screening insurance
#     portfolio results and visualisations.
#
# =============================================================================
...
if is_insurance_section:
    with tab_insurance:
        ...
        st.write(
            "Insurance portfolio analysis integrated from the final CKD Linear "
            "SVM Early Screening validation results."
        )

        st.info(
            "These portfolio results are the Early Screening validation outputs "
            "from the project notebook. They are presented here without requiring "
            "a separate training/testing dataset at runtime."
        )

        # ---------------------------------------------------------------------
        # EARLY-SCREENING NOTEBOOK SUMMARY
        # ---------------------------------------------------------------------
        total_members = 40000
        low_risk_members = 29945
        high_risk_members = 10055
        insured_members = 25809
        uninsured_members = 14191
        high_risk_insured = 6249
        low_risk_insured = 19560
        high_risk_uninsured = 3806
        low_risk_uninsured = 10385

        # ---------------------------------------------------------------------
        # KPI SUMMARY
        # ---------------------------------------------------------------------
        st.divider()
        st.subheader("📊 Early Screening Insurance Portfolio")

        c1, c2, c3, c4, c5 = st.columns(5)
        with c1:
            st.metric("Validation Members", f"{total_members:,}")
        with c2:
            st.metric("Insured", f"{insured_members:,} (64.52%)")
        with c3:
            st.metric("Uninsured", f"{uninsured_members:,} (35.48%)")
        with c4:
            st.metric("High CKD Risk", f"{high_risk_members:,} (25.14%)")
        with c5:
            st.metric("High Risk + No Insurance", f"{high_risk_uninsured:,} (9.52%)")
        ...
        with insight_col1:
            st.markdown(
                ...
                <div class="metric-card">
                    ...
                    <div class="metric-value">{high_risk_members / total_members * 100:.2f}%</div>
                    <div class="metric-note">{high_risk_members:,} of {total_members:,} validation members are predicted high risk.</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        with insight_col2:
            st.markdown(
                f"""
                <div class="metric-card">
                    <div class="metric-label">Uninsured High-Risk Segment</div>
                    <div class="metric-value">{high_risk_uninsured / total_members * 100:.2f}%</div>
                    <div class="metric-note">{high_risk_uninsured:,} members are both uninsured and predicted high risk.</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        with insight_col3:
            st.markdown(
                f"""
                <div class="metric-card">
                    <div class="metric-label">High-Risk Rate Among Uninsured</div>
                    <div class="metric-value">{high_risk_uninsured / uninsured_members * 100:.2f}%</div>
                    <div class="metric-note">This is higher than the insured high-risk rate of {high_risk_insured / insured_members * 100:.2f}%.</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
        ...
        st.caption(
            "The figures above reproduce the Early Screening notebook's commercial and portfolio calculations."
        )

        # ---------------------------------------------------------------------
        # MARKET SEGMENTATION TABLE + PLOT
        # ---------------------------------------------------------------------
        st.divider()
        st.subheader("🛡️ Predicted CKD Risk by Health Insurance")

        segment_table = pd.DataFrame(
            {
                "Low CKD Risk": [low_risk_insured, low_risk_uninsured],
                "High CKD Risk": [high_risk_insured, high_risk_uninsured],
            },
            index=["Has Insurance", "No Insurance"],
        )
        segment_percent = pd.DataFrame(
            {
                "Low CKD Risk": [75.79, 73.18],
                "High CKD Risk": [24.21, 26.82],
            },
            index=["Has Insurance", "No Insurance"],
        )
        ...
        with left:
            ...
            st.caption("Counts from the Early Screening validation cohort.")
            st.dataframe(segment_percent, use_container_width=True)
            st.caption("Row percentages: predicted CKD risk within each insurance group.")
        with right:
            st.image(
                BytesIO(__import__("base64").b64decode("")),
                use_container_width=True,
            )
        ...
        risk_distribution = pd.DataFrame(
            {"Risk Level": ["Low Risk", "High Risk"], "Members": [low_risk_members, high_risk_members]}
        )
        ...
        with r2:
            st.image(
                BytesIO(__import__("base64").b64decode("")),
                use_container_width=True,
            )
        ...
        # ---------------------------------------------------------------------
        st.divider()
        st.subheader("🏥 Insurance Portfolio Breakdown")
        ip1, ip2 = st.columns([1, 1.35])
        with ip1:
            st.markdown("**Portfolio counts**")
            st.dataframe(segment_table.rename_axis("Health Insurance"), use_container_width=True)
            st.markdown("**Portfolio percentages**")
            st.dataframe(segment_percent.rename_axis("Health Insurance"), use_container_width=True)
        with ip2:
            st.image(
                BytesIO(__import__("base64").b64decode("")),
                use_container_width=True,
            )

        # ---------------------------------------------------------------------
        # EXECUTIVE SUNBURST — DIRECT NOTEBOOK FIGURE
        # ---------------------------------------------------------------------
        st.divider()
        st.subheader("⭕ Executive Insurance Portfolio — Circular Analysis")
        st.caption("Directly integrated from the Early Screening notebook's executive Sunburst visualisation.")
        insurance_sunburst_spec = __import__("json").loads("""{"config":{"plotlyServerURL":"https://plot.ly"},"data":[{"branchvalues":"total","customdata":[["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","(?)"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","High Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"],["(?)","(?)","(?)","Low Risk"]],"domain":{"x":[0,1],"y":[0,1]},"hovertemplate":"<b>%{label}</b><br>Members: %{value}<br>Parent: %{percentParent:.1%}<br>Root: %{percentRoot:.1%}<extra></extra>","ids":["Australia/Has Insurance","UK/Has Insurance","USA/Has Insurance","India/Has Insurance","Australia/No Insurance","India/No Insurance","UK/No Insurance","Other/Has Insurance","Canada/Has Insurance","USA/No Insurance","Canada/No Insurance","Other/No Insurance","Australia","UK","USA","India","Other","Canada","Australia/Has Insurance/1/High Risk/Middle","USA/Has Insurance/1/High Risk/High","USA/No Insurance/1/High Risk/Low","UK/No Insurance/1/High Risk/Low","India/No Insurance/1/High Risk/Middle","UK/No Insurance/1/High Risk/Middle","Australia/Has Insurance/1/High Risk/Low","USA/No Insurance/1/High Risk/High","India/Has Insurance/1/High Risk/Middle","USA/No Insurance/1/High Risk/Middle","Canada/No Insurance/1/High Risk/Middle","USA/Has Insurance/1/High Risk/Low","USA/Has Insurance/1/High Risk/Middle","India/No Insurance/1/High Risk/Low","Canada/Has Insurance/1/High Risk/Middle","UK/Has Insurance/1/High Risk/High","Other/Has Insurance/1/High Risk/Middle","Australia/Has Insurance/1/High Risk/High","UK/Has Insurance/1/High Risk/Middle","Canada/Has Insurance/1/High Risk/High","Other/No Insurance/1/High Risk/Middle","India/Has Insurance/1/High Risk/High","Other/Has Insurance/1/High Risk/Low","Other/No Insurance/1/High Risk/Low","UK/Has Insurance/1/High Risk/Low","India/Has Insurance/1/High Risk/Low","Canada/No Insurance/1/High Risk/Low","Australia/No Insurance/1/High Risk/Middle","Other/Has Insurance/1/High Risk/High","Australia/No Insurance/1/High Risk/Low","Canada/No Insurance/1/High Risk/High","India/No Insurance/1/High Risk/High","Other/No Insurance/1/High Risk/High","Canada/Has Insurance/1/High Risk/Low","UK/No Insurance/1/High Risk/High","Australia/No Insurance/1/High Risk/High","Australia/Has Insurance/1/High Risk","USA/Has Insurance/1/High Risk","USA/
Risk","India/Has Insurance/0/Low Risk","USA/No Insurance/0/Low Risk","Canada/No Insurance/0/Low Risk","Canada/Has Insurance/0/Low Risk","Other/Has Insurance/0/Low Risk","Australia/Has Insurance/0/Low Risk","Australia/Has Insurance/0/Low Risk","Other/No Insurance/0/Low Risk","Other/No Insurance/0/Low Risk","Canada/No Insurance/0/Low Risk","UK/No Insurance/0/Low Risk","Australia/No Insurance/0/Low Risk","Other/No Insurance/0/Low Risk","Australia/Has Insurance/0","UK/Has Insurance/0","USA/Has Insurance/0","India/Has Insurance/0","Australia/No Insurance/0","India/No Insurance/0","UK/No Insurance/0","Other/Has Insurance/0","Canada/Has Insurance/0","USA/No Insurance/0","Canada/No Insurance/0","Other/No Insurance/0","Australia/Has Insurance","UK/Has Insurance","USA/Has Insurance","India/Has Insurance","Australia/No Insurance","India/No Insurance","UK/No Insurance","Other/Has Insurance","Canada/Has Insurance","USA/No Insurance","Canada/No Insurance","Other/No Insurance"],"textinfo":"label+percent parent","type":"sunburst","values":{"bdata":"","dtype":"i2"}}],"layout":{"font":{"family":"Arial","size":15},"height":850,"legend":{"tracegroupgap":0},"margin":{"b":20,"l":20,"r":20,"t":70},"template":{"data":{"bar":[{"error_x":{"color":"#2a3f5f"},"error_y":{"color":"#2a3f5f"},"marker":{"line":{"color":"white","width":0.5},"pattern":{"fillmode":"overlay","size":10,"solidity":0.2}},"type":"bar"}],"barpolar":[{"marker":{"line":{"color":"white","width":0.5},"pattern":{"fillmode":"overlay","size":10,"solidity":0.2}},"type":"barpolar"}],"carpet":[{"aaxis":{"endlinecolor":"#2a3f5f","gridcolor":"#C8D4E3","linecolor":"#C8D4E3","minorgridcolor":"#C8D4E3","startlinecolor":"#2a3f5f"},"baxis":{"endlinecolor":"#2a3f5f","gridcolor":"#C8D4E3","linecolor":"#C8D4E3","minorgridcolor":"#C8D4E3","startlinecolor":"#2a3f5f"},"type":"carpet"}],"choropleth":[{"colorbar":{"outlinewidth":0,"ticks":""},"type":"choropleth"}],"contour":[{"colorbar":{"outlinewidth":0,"ticks":""},"colorscale":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"type":"contour"}],"contourcarpet":[{"colorbar":{"outlinewidth":0,"ticks":""},"type":"contourcarpet"}],"heatmap":[{"colorbar":{"outlinewidth":0,"ticks":""},"colorscale":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"type":"heatmap"}],"histogram":[{"marker":{"pattern":{"fillmode":"overlay","size":10,"solidity":0.2}},"type":"histogram"}],"histogram2d":[{"colorbar":{"outlinewidth":0,"ticks":""},"colorscale":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"type":"histogram2d"}],"histogram2dcontour":[{"colorbar":{"outlinewidth":0,"ticks":""},"colorscale":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"type":"histogram2dcontour"}],"mesh3d":[{"colorbar":{"outlinewidth":0,"ticks":""},"type":"mesh3d"}],"parcoords":[{"line":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"parcoords"}],"pie":[{"automargin":true,"type":"pie"}],"scatter":[{"fillpattern":{"fillmode":"overlay","size":10,"solidity":0.2},"type":"scatter"}],"scatter3d":[{"line":{"colorbar":{"outlinewidth":0,"ticks":""}},"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scatter3d"}],"scattercarpet":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scattercarpet"}],"scattergeo":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scattergeo"}],"scattergl":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scattergl"}],"scattermap":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scattermap"}],"scattermapbox":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scattermapbox"}],"scatterpolar":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scatterpolar"}],"scatterpolargl":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scatterpolargl"}],"scatterternary":[{"marker":{"colorbar":{"outlinewidth":0,"ticks":""}},"type":"scatterternary"}],"surface":[{"colorbar":{"outlinewidth":0,"ticks":""},"colorscale":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"type":"surface"}],"table":[{"cells":{"fill":{"color":"#EBF0F8"},"line":{"color":"white"}},"header":{"fill":{"color":"#C8D4E3"},"line":{"color":"white"}},"type":"table"}]},"layout":{"annotationdefaults":{"arrowcolor":"#2a3f5f","arrowhead":0,"arrowwidth":1},"autotypenumbers":"strict","coloraxis":{"colorbar":{"outlinewidth":0,"ticks":""}},"colorscale":{"diverging":[[0,"#8e0152"],[0.1,"#c51b7d"],[0.2,"#de77ae"],[0.3,"#f1b6da"],[0.4,"#fde0ef"],[0.5,"#f7f7f7"],[0.6,"#e6f5d0"],[0.7,"#b8e186"],[0.8,"#7fbc41"],[0.9,"#4d9221"],[1,"#276419"]],"sequential":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]],"sequentialminus":[[0,"#0d0887"],[0.1111111111111111,"#46039f"],[0.2222222222222222,"#7201a8"],[0.3333333333333333,"#9c179e"],[0.4444444444444444,"#bd3786"],[0.5555555555555556,"#d8576b"],[0.6666666666666666,"#ed7953"],[0.7777777777777778,"#fb9f3a"],[0.8888888888888888,"#fdca26"],[1,"#f0f921"]]},"colorway":["#636efa","#EF553B","#00cc96","#ab63fa","#FFA15A","#19d3f3","#FF6692","#B6E880","#FF97FF","#FECB52"],"font":{"color":"#2a3f5f"},"geo":{"bgcolor":"white","lakecolor":"white","landcolor":"white","showlakes":true,"showland":true,"subunitcolor":"#C8D4E3"},"hoverlabel":{"align":"left"},"hovermode":"closest","mapbox":{"style":"light"},"paper_bgcolor":"white","plot_bgcolor":"white","polar":{"angularaxis":{"gridcolor":"#EBF0F8","linecolor":"#EBF0F8","ticks":""},"bgcolor":"white","radialaxis":{"gridcolor":"#EBF0F8","linecolor":"#EBF0F8","ticks":""}},"scene":{"xaxis":{"backgroundcolor":"white","gridcolor":"#DFE8F3","gridwidth":2,"linecolor":"#EBF0F8","showbackground":true,"ticks":"","zerolinecolor":"#EBF0F8"},"yaxis":{"backgroundcolor":"white","gridcolor":"#DFE8F3","gridwidth":2,"linecolor":"#EBF0F8","showbackground":true,"ticks":"","zerolinecolor":"#EBF0F8"},"zaxis":{"backgroundcolor":"white","gridcolor":"#DFE8F3","gridwidth":2,"linecolor":"#EBF0F8","showbackground":true,"ticks":"","zerolinecolor":"#EBF0F8"}},"shapedefaults":{"line":{"color":"#2a3f5f"}},"ternary":{"aaxis":{"gridcolor":"#DFE8F3","linecolor":"#A2B1C6","ticks":""},"baxis":{"gridcolor":"#DFE8F3","linecolor":"#A2B1C6","ticks":""},"bgcolor":"white","caxis":{"gridcolor":"#DFE8F3","linecolor":"#A2B1C6","ticks":""}},"title":{"x":0.05},"xaxis":{"automargin":true,"gridcolor":"#EBF0F8","linecolor":"#EBF0F8","ticks":"","title":{"standoff":15},"zerolinecolor":"#EBF0F8","zerolinewidth":2},"yaxis":{"automargin":true,"gridcolor":"#EBF0F8","linecolor":"#EBF0F8","ticks":"","title":{"standoff":15},"zerolinecolor":"#EBF0F8","zerolinewidth":2}}},"title":{"text":"<b>Executive Insurance Portfolio Analysis</b>","x":0.5},"width":1300}}""")
        insurance_sunburst = go.Figure(insurance_sunburst_spec, skip_invalid=True)
        st.plotly_chart(insurance_sunburst, use_container_width=True, key="insurance_intelligence_sunburst")

        # ---------------------------------------------------------------------
        # INSURANCE PORTFOLIO RISK LANDSCAPE — NOTEBOOK VISUAL
        # ---------------------------------------------------------------------
        st.divider()
        st.subheader("🌐 Insurance Portfolio Risk Landscape")
        st.caption(
            "Income is shown against age; marker size represents BMI and the portfolio segment "
            "is the Early Screening risk/insurance combination."
        )
        st.image(
            BytesIO(__import__("base64").b64decode("")),
            use_container_width=True,
        )
        ...
        h1, h2 = st.columns(2)
        with h1:
            st.metric("High CKD Risk + No Insurance", f"{high_risk_uninsured:,}")
        with h2:
            st.metric("Potential High-Priority Segment", f"{high_risk_uninsured / total_members * 100:.2f}%")

        st.warning(
            "This segment is shown for population-level analytical segmentation only. "
            "It must not be used to deny, restrict, price, or otherwise determine an individual's "
        ...
        # ---------------------------------------------------------------------
        st.caption(
            "Source: Early Screening validation outputs from early_screening(2).ipynb — "
            "the notebook builds the insurance portfolio directly from x_valid and y_pred_svm."
        )
